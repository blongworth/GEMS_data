...
Ar loss corrected, init only

```{r}
rate_sum %>% 
  select(experiment, dn_init_cor) %>% 
  pivot_longer(c(dn_init_cor)) %>% 
  ggplot(aes(experiment, value, fill = replicate)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "DN rate at O2 min (Ar corrected)",
       y = "15N2 production (umol/day)")
...
```{r}
ggplot(rate_sum, aes(experiment, dn_init_cor)) +
  geom_boxplot() +
  labs(title = "DN rate at O2 min (Ar corrected)",
       y = "15N2 production (umol/day)")
```
...
```{r}
ggplot(rate_sum, aes(grass, dn_init_cor)) +
  geom_boxplot() +
  labs(title = "DN rate at O2 min (Ar corrected)",
       y = "15N2 production (umol/day)")
```
...
```{r}
rate_sum |> 
  select(experiment, replicate, grass, open, dn_rate = dn_init) |> 
  write_csv("SMURP_DN_rates.csv")
```
...
```{r}
ggplot(rate_sum, aes(experiment, norm_rate)) +
  geom_boxplot() +
  labs(title = "Scaled DN rate at O2 min (Ar corrected)",
       subtitle = "Rates scaled to a range of 0-1 by replicate",
       y = "Scaled 15N2 production (0-1, by replicate)") 
```
```{r}
rate_sum %>% 
  select(experiment, norm_rate) %>% 
  pivot_longer(c(norm_rate)) %>% 
  ggplot(aes(experiment, value, fill = replicate)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "DN rate at O2 min (Ar corrected)",
       y = "15N2 production (umol/day)")
